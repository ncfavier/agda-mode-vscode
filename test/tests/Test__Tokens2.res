...
describe("Tokens", ~timeout=10000, () => {
  ...
  let acquire = filepath => {
    ...
    | Some(_channels) =>
      ...
      ->Promise.flatMap(result =>
        ...
        | Some(Ok(state)) => Promise.resolved(Ok(state))
        | Some(Error(error)) =>
  ...
  describe("GotoDefinition.agda", () => {
    ...
    Q.it("should produce correct tokens after inserting a space", () => {
      acquire("GotoDefinition.agda")
      ->Promise.flatMapOk(state => {
        let position = VSCode.Position.make(6, 7)
        Editor.Text.insert(state.document, position, " ")->Promise.map(_ => Ok(state))
      })
      ->Promise.flatMapOk(state => {
        let tokens =
          state.tokens
